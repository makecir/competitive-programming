...
int main(){
	...
	rep(i,n)rep(j,n){
		cin>>v[i*n+j];
		v[i*n+j]--;
		bd[v[i*n+j]/n][v[i*n+j]%n]=(i*n)+j+1;
	}
	rep(i,n)puta(bd[i]);
	Dijkstra<ll> g(n*n+1);
	rep(i,n)rep(j,n){
		...
		g.add(a,b,(a>b?0:1),true);
		g.add(b,a,(b>a?0:1),true);
		if(i==n-1) b=0;
		else b=bd[i+1][j];
		g.add(a,b,(a>b?0:1),true);
		g.add(b,a,(b>a?0:1),true);
	...
	rep(i,n*n){
		...
		tmp=g.dist(tar,0);
		ans+=tmp;
		//g.add(tar,0,tmp,true);
		ll y=v[i]/n,x=v[i]%n;
		puta(tar,tmp);
		rep(j,4){
			ll ty=y+dy[j],tx=x+dx[j];
			if(tx<0||n<=tx||ty<0||n<=ty)continue;
			//if(bd[y][x]>bd[ty][tx])g.add(bd[ty][tx]+1,0,tmp,true);
		}
	}
	puta(ans);
}
