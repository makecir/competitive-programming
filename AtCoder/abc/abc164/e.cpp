...
int main(){
	...
	rep(i,n){
		rep(j,nmx){
			ll afc=min(j+c[i],nmx);
			g.add(i+j*n,i+afc*n,d[i],true);
		}
	}
	rep(i,m){
		rep(j,nmx+1){
			if(j-a[i]<0)continue;
			g.add(u[i]+n*j,v[i]+n*(j-a[i]),b[i],true);
			g.add(v[i]+n*j,u[i]+n*(j-a[i]),b[i],true);
		}
	}
	vl dist=g.dist(s*n);
	vl ans(n,LINF);
	rep(i,n){
		rep(j,nmx+1)chmin(ans[i],dist[i+j*n]);
	}
