...
int main() {
    ...
    dsu d(h * w);
    rep(i, h) rep(j, w) {
        if(j + 1 < w && vv[i][j] && vv[i][j + 1])
            d.merge(cas(i, j), cas(i, j + 1));
        if(i + 1 < h && vv[i][j] && vv[i + 1][j])
            d.merge(cas(i, j), cas(i + 1, j));
    }
    bool ans = false;
    rep(i, 4) rep(j, 4) {
        if(i == j)
        ...
        if(xj < 0 || xj >= w)
            continue;
        ans |= d.same(cas(yi, xi), cas(yj, xj));
    }
    Yn(ans);
}
