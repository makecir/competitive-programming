...
int main() {
    ...
    vl v(n);
    rep(i, n) cin >> v[i];
    fenwick_tree<ll> cnt(200010);
    fenwick_tree<ll> sum(200010);
    ...
    rep(i, n - 1) {
        mint k = i + 2;
        ans[i + 1] = (ans[i] * (k - 1) * (k - 1) / k / k);
        ans[i + 1] += (v[i + 1] / k / k);
        ans[i + 1] += (cnt.sum(0, v[i + 1] + 1) * v[i + 1] +
                       sum.sum(v[i + 1] + 1, 200010)) *
                      2 / k / k;
        cnt.add(v[i + 1], 1);
        sum.add(v[i + 1], v[i + 1]);
    }
    rep(i, n) { puta(ans[i].val()); }
}
