...
int main() {
    ...
    map<ll, ll> mp;
    mp[0] = 0;
    function<ll(ll, ll)> rec = [&](ll rem, ll pos) {
        if(mp.count(rem))
            return mp[rem];
        if(pos == n - 1) {
            return mp[rem] = rem / a[pos];
        }
        ll ret = 0;
        ll mn = (rem % a[pos + 1]) / a[pos] +
                rec(rem / a[pos + 1] * a[pos + 1], pos + 1);
        ll mx = (a[pos + 1] - (rem % a[pos + 1])) / a[pos] +
                rec((rem / a[pos + 1] + 1) * a[pos + 1], pos + 1);
