...
int main()
{
    ...
    vl a(n);
    vl sm(n + 1);
    rep(i, n) cin >> a[i], sm[i + 1] = sm[i] + a[i];
    vvl vv(10, vl(n + 1, 0));
    rep(i, n)
    {
        // a[i] の桁数を求める
    ...
    rep(i, n)
    {
        ...
        rep(j, 10)
        {
            tab *= 10;
            tab %= MOD;
            ans += tab * (vv[j][n] - vv[j][i + 1]);
            // puta(tab, (vv[j][n] - vv[j][i + 1]), tab * (vv[j][n] - vv[j][i + 1]));
            ans %= MOD;
        }
        // puta(ans);
        // puta("----");
