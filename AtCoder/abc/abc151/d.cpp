...
int main(){
	...
	vvl dist((h+2)*(w+2),vl((h+2)*(w+2),LINF));
	rep(i,(h+2)*(w+2)){
		dist[i][i]=0;
	}
	rep(i,h+2){
		rep(j,w+1){
			if(vv[i][j]&&vv[i][j+1]){
				dist[i*(w+2)+j][i*(w+2)+j+1]=1;
				dist[i*(w+2)+j+1][i*(w+2)+j]=1;
			}
		}
	}
	rep(j,w+2){
		rep(i,h+1){
			if(vv[i][j]&&vv[i+1][j]){
				dist[i*(w+2)+j][(i+1)*(w+2)+j]=1;
				dist[(i+1)*(w+2)+j][i*(w+2)+j]=1;
			}
		}
	}
	rep(k,(h+2)*(w+2))rep(i,(h+2)*(w+2))rep(j,(h+2)*(w+2)){
		chmin(dist[i][j],dist[i][k]+dist[k][j]);
	}
	ll ans=0;
	rep(i,(h+2)*(w+2)){
		rep(j,(h+2)*(w+2)){
			if(dist[i][j]>10000)continue;
			chmax(ans,dist[i][j]);
		}
	}
	puta(ans);
