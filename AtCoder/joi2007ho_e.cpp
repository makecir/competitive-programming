...
vl ld(100),rd(100),lm(100),rm(100);

ll rec(int s){
	ll lma=1,rma=1;
	if(lm[s]!=0)lma=rec(lm[s]-1);
	if(rm[s]!=0)rma=rec(rm[s]-1);
	ll lc=(lma*ld[s])*(rma*rd[s])/gcd((lma*ld[s]),(rma*rd[s]));
	return lc/ld[s]+lc/rd[s];
}
...
int main(){
	...
	vb us(n,true);
	rep(i,n){
		cin>>ld[i]>>rd[i]>>lm[i]>>rm[i];
		if(lm[i])us[lm[i]-1]=false;
		if(rm[i])us[rm[i]-1]=false;
	}
	int st=-1;
	rep(i,n)if(us[i])st=i;
	ll ans=rec(st);
	cout<<ans<<endl;
}
