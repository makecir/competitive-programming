...
int main(){
	...
	rep(i,n) bestv+=(dist[ord[i]][ord[(i+1)%n]]-(best/n))*(dist[ord[i]][ord[(i+1)%n]]-(best/n));
	rep(i,n)out[i]=ord[i];
	...
	while(ti<1500,st++<stage){
		...
		rep(i,n-2){
			range(j,i+2,n){
				double be=0,af=0;
				be=abs(dist[ord[i]][ord[i+1]]-dist[ord[j]][ord[(j+1)%n]]);
				af=abs(dist[ord[i]][ord[j]]-dist[ord[i+1]][ord[(j+1)%n]]);
