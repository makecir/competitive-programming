...
int main(){
	...
	Dijkstra g(n*3);
	rep(i,n-1){
		g.add(i,i+1,abs(v[i]-v[i+1]));
		g.add(i+n,i+1+n,abs(v[n-i-1]-v[n-i-2]));
		g.add(i+n+n,i+1+n+n,abs(v[i]-v[i+1]));
	}
	rep(i,n-1){
		g.add(i,i+1+n,abs(v[i]-v[n-i-2]));
		g.add(i+n,i+1+n+n,abs(v[n-i-1]-v[i+1]));
		puta(v[i],v[n-i-2],v[n-i-1],v[i+1]);
	}
	ll ans=LINF;
	rep(i,3){
		puta(g.dist(0,n*(i+1)-1));
		chmin(ans,g.dist(0,n*(i+1)-1));
	}
	puta(ans);
}
