...
int main(){
	...
	ll n;
	pll s,t;
	cin>>s.fs>>s.sc>>t.fs>>t.sc;
	cin>>n;
	Dijkstra<double> g(n+2);
	vd x(n),y(n),r(n);
	rep(i,n)cin>>x[i]>>y[i]>>r[i];
	g.add(n,n+1,hypot(t.fs-s.fs,t.sc-s.sc));
	rep(i,n){
		g.add(i,n,max(0.0,hypot(x[i]-s.fs,y[i]-s.sc)-r[i]));
		g.add(i,n+1,max(0.0,hypot(x[i]-t.fs,y[i]-t.sc)-r[i]));
		rep(j,i){
			g.add(i,j,max(0.0,hypot(x[i]-x[j],y[i]-y[j])-r[i]-r[j]));
		}
	}
	double ans=g.dist(n,n+1);
	fcout(10)<<ans<<endl;
}
