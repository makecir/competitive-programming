...
int main(){
	...
	rep(q,q_mx){
		...
		// input
		int si,sj,ti,tj,ci,cj,ni,nj,cost;
		cin>>si>>sj>>ti>>tj;
		...
		while(!que.empty()){
			...
			rep(dir,4){
				...
				if(dir==0)cost+=h_cost[ci][cj];
				if(dir==1)cost+=v_cost[ci][cj];
				if(dir==2)cost+=h_cost[ci][cj-1];
				if(dir==3)cost+=v_cost[ci-1][cj];
		...
		for(auto d:op){
			...
			ci+=dy[dir];cj+=dx[dir];
			tar=(tar*sz(op)+ave*(ver?abs(ci-si):abs(cj-sj)))/(sz(op)+(ver?abs(ci-si):abs(cj-sj)))+1;
		}
	}
}
