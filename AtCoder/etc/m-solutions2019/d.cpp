...
int main(){
	...
	vl c(n),ansv(n);
	vb used(n,false);
	auto comp = [](tlll l,tlll r){
		if(get<0>(l)!=get<0>(r)) return get<0>(l) < get<0>(r);
		else return get<1>(l) < get<1>(r);
	};
	priority_queue<tlll,vector<tlll>,decltype(comp)> p(comp);

	vl ch(n);
	rep(i,n-1){
		int ina,inb;
		cin>>ina>>inb;ina--;inb--;
		g.add(ina,inb,0,false);
		ch[ina]++;ch[inb]++;
	}
	rep(i,n){
		if(ch[i]==1){
			p.push(tlll(ch[i],0,i));
			used[i]=true;
		}
	}
	rep(i,n)cin>>c[i];
	...
	while(!p.empty()){
		auto t=p.top();p.pop();
		ansv[get<2>(t)]=c[i];i++;
		for(auto x:g.edge[get<2>(t)]){
			if(used[get<0>(x)]==false){
				ch[get<0>(x)]--;
				if(ch[get<0>(x)]==1){
					p.push(tlll(ch[get<0>(x)],0,get<0>(x)));
					used[get<0>(x)]=true;
				}
			}
		}
