...
int main(){
	...
	vi temp(n);
	rep(i,n)cin>>temp[i];
	Dijkstra<int> g(n*x*2);
	rep(i,m){
		cin>>a>>b>>d;
		a--;b--;
		if(temp[b]==0){
			rep(j,x){
				g.add(j*n+a,(x-1)*n+b,d,true);
			}
			g.add(a+n*x,(x-1)*n+b,d,true);
		}
		else if(temp[b]==2){
			rep(j,x){
				g.add(j*n+a+n*x,(x-1)*n+b+n*x,d,true);
			}
			g.add(a,(x-1)*n+b+n*x,d,true);
		}
		else{
			rep(k,2){
				rep(j,x){
					if(j-d<=0)
					g.add(j*n+a+k*n*x,max(0ll,j-d)*n+b+k*n*x,d,true);
				}
			}
		}
		swap(a,b);
	...
	int ans=INF;
	auto dist=g.dist((x-1)*n);
	rep(i,x*2)chmin(ans,dist[i*n+n-1]);
	rep(i,x*2){
		rep(j,n)cout<<dist[i*n+j]<<" ";
		cout<<endl;
	}
	puta(ans);
}
